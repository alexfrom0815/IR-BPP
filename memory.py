...
class ReplayMemory():
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    while not valid:
      ...
      if (self.transitions.index - idx) % self.capacity > self.n and (idx - self.transitions.index) % self.capacity >= 1 and prob != 0:
    ...
    next_state = torch.stack([trans.state for trans in transition[self.n:self.n + 1 ]]).to(device=self.device).to(dtype=torch.float32)

    # Discrete action to be used as index
    action = torch.tensor([transition[0].action], dtype=torch.int64, device=self.device)
    # Calculate truncated n-step discounted return R^n = Σ_k=0->n-1 (γ^k)R_t+k+1 (note that invalid nth next states have reward 0)
    R = torch.tensor([sum(self.discount ** n * transition[n].reward for n in range(self.n))], dtype=torch.float32, device=self.device)
    # 并不是中间的state都用到了，只是用这中间的state算return
    # Mask for non-terminal nth next states
    nonterminal = torch.tensor([transition[self.n].nonterminal], dtype=torch.float32, device=self.device)
