...
import numpy as np
from tools import init, SupConLoss
...
class DQNP(nn.Module):
  def __init__(self, args, action_space, shapeArray):
    ...
    self.criterion = SupConLoss(temperature=0.07, device=args.device,
                                contrast_mode = 'one')
  ...
  def embed_physic_only_with_heightmap_contrastive(self, x, getCL = False):
      ...
      next_item_ID = next_item[:, 0].long()
      assert self.args.shapePreType == 'SurfacePointsRandom' or self.args.shapePreType == 'SurfacePointsEncode'
      ...
      if getCL:
        ...
        # info nce loss
        # loss_cl = self.criterion(torch.cat((shape_feature.unsqueeze(1), shape_feature_CL.unsqueeze(1)), dim=1))
        # sup con for instance
        loss_cl = self.criterion(torch.cat((shape_feature.unsqueeze(1), shape_feature_CL.unsqueeze(1)), dim=1), next_item_ID)

        # logits_cl, labels_cl = info_nce_loss(torch.cat((shape_feature, shape_feature_CL), dim=0))
      ...
      return embeddings, graph_embed, loss_cl

  def forward(self, x, log=False,  getCL = False):

      loss_cl = None
      if self.args.previewNum > 1:
        # x, xGlobal = self.embed_physic_k_shape(x)
        x, xGlobal = self.embed_physic_k_shape_with_gat(x)
      else:
        # x, xGlobal = self.embed_physic_only_with_heightmap_fixed_indices(x)
        x, xGlobal = self.embed_physic_only_with_heightmap(x)
        # x, xGlobal = self.embed_physic_only_with_heightmap_advantage(x)
        # x, xGlobal, loss_cl = self.embed_physic_only_with_heightmap_contrastive(x, getCL)
